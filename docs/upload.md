...
- `waf_simulator.py`: Main security engine and proxy (`Port 8080`).
- `demo_app.py`: Protected target backend service (`Port 5001`).
- `dashboard.py`: Visualization and monitoring interface (`Port 8501`).

**CLI Tools**

- `tools/evaluate.py`: Automated performance evaluation suite (no port assignment).
...
2. **Heuristic Fallback:** If the model is absent, inference fails, or distribution shift makes ML results uncertain, the system defaults to keyword-based detection. This ensures continuous security coverage and provides human-interpretable reasons for blocked traffic.
...
1. **Primary:** Attempts to parse the local JSONL log file for the most granular event history.
2. **Fallback:** If the log file is inaccessible, queries the WAF APIs (`/metrics` and `/events`) directly.
...
- **Recall (0.75):** While effective, the system missed 25% of malicious payloads. These 2 false negatives indicate that certain sophisticated or novel payloads bypassed lexical features, suggesting a need for expanded rules or model retraining.
...
- **End-to-End Observability:** Integration of JSONL logs and real-time visualization allows immediate incident response.
...
- **Lack of Deobfuscation:** The prototype lacks a normalization layer; attackers could use complex encoding (e.g., nested Base64 or URL encoding) to evade detection.
- **Static Intelligence:** The ML model is static. Without an automated retraining pipeline, detection capability will degrade as new attack variants emerge.
...
1. **Signature/Rule Engine Integration:** Incorporate established rule sets, such as the OWASP Core Rule Set (`CRS`), to supplement ML detection.
2. **Automated Retraining Pipelines:** Establish a workflow to automatically retrain the ML model using new, verified logs to improve recall.
3. **Payload Normalization:** Implement a pre-processing stage to decode and deobfuscate payloads before they reach the classification engine.
4. **Advanced Performance Benchmarking:** Stress-test the system to identify p99 latency and throughput limits under high-concurrency loads.
...
### Q1. Why combine machine learning with heuristics?

ML identifies complex, learned patterns, but it can fail due to distribution shift. Heuristics provide a reliable, interpretable fallback, ensuring the system remains functional even if the model is missing or traffic deviates from the training set.

### Q2. What is the risk of the 2 false negatives in your results?

False negatives are missed attacks that reach the backend. In our tests, `FN = 2`, meaning some malicious payloads were not caught. This indicates the need for more diverse feature extraction or a broader signature-based rule set.

### Q3. How was WAF effectiveness measured?

A confusion matrix was used to calculate accuracy, precision, recall, and F1 score. Average latency was also measured to ensure the security layer does not cause significant performance degradation for the user.

### Q4. Is this system ready for a production environment?

No. This is an academic simulation. A production-grade WAF requires TLS termination, full protocol semantics parsing, hardened deployment environments, and deep payload deobfuscation to handle real-world evasion techniques.

### Q5. What is the key contribution of this architecture?

The primary contribution is the creation of an end-to-end, measurable security pipeline that integrates detection, proxying, and real-time visualization into a single, reproducible stack.

## 13. Appendix B: Demo & Execution Guide

### 13.1 Local Execution (Three-Terminal Setup)
